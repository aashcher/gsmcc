...
function [W] = calc_emi1D_h(V)
    ...
    for k=1:ns
        ...
        for i=1:no
            ...
            ff(3,i) = ta(3,i);
            ta(2,i) = kz(1,i)*( V((k-1)*2*no+i+no,1) - V((k-1)*2*no+i,1) );
            ff(2,i) = -ta(2,i);
        end
        ta = fft(ta,N,2);
        for i=1:N
            ...
            tvar = ta(2,i);
            ta(2,i) = (el(1,k)/eb)*tvar*MG(2,k,i) + ta(3,i)*MG(3,k,i);
            ta(3,i) = tvar*MG(3,k,i) - (eb/el(1,k))*ta(3,i)*MG(2,k,i);
        end
        ta = ifft(ta,N,2);
        tvar = -0.5*1i*cwz(2,k);
        for i=1:no
            ff(1,i) = ff(1,i) + ta(1,i);
            ff(2,i) = ff(2,i) + ta(2,i);
            ff(3,i) = ff(3,i) + ta(3,i);
            W((k-1)*2*no+i,1) = -( eb*ff(1,i) + ky(1,i)*ff(3,i)  )/kz(1,i);
