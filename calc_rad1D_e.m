...
function [W] = calc_rad1D_e(V)
    ...
    for i = 1:no
        ...
        for k = 1:ns
            kk = (k-1)*3*no;
            ta(1,k) = ( ky(1,i)*V(kk+2*no+i,1) - V(kk+i,1) )/kz(1,i);
		...
		for k=1:(ns-1)
			...
			W((k-1)*3*no+i,1) = W((k-1)*3*no+i,1) + eb*tvar1;
			...
			W((ns-k)*3*no+i,1) = W((ns-k)*3*no+i,1) + eb*tvar1;
